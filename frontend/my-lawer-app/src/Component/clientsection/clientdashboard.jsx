...
const ClientPortal = () => {
  ...
  useEffect(() => {
    const fetchDocumentRequests = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error("Unauthorized or error fetching document requests");
        }
  ...
  //   const handleUploadClick = async (docId) => {
  //     if (!selectedFile) {
  ...
  //       const response = await axios.post(`/api/upload/${docId}`, formData, {
  //         headers: {
